...
it("backs up committed WAL content before upgrading an older database", async () => {
  ...
  try {
    ...
    expect(old.prepare("PRAGMA user_version").get()?.user_version).toBe(2);
    expect(old.prepare("SELECT version FROM schema_migrations ORDER BY version").all().map((row) => row.version)).toEqual([1, 2]);
