...
只读清单现在通过 DescribeDocuments、DescribeChunks 和 DescribeChunk 获取；列表预览不用于计算完整内容哈希。用户在设置中核对文档来源类型与人物阶段后，才启用对应资料包。

生产清单使用 `DETAIL` 版本来源：检索命中先核对已确认内容 SHA-256，再读取同一切片详情，核对知识库、文档、切片归属、启用状态、详情时间原值和完整文本。这样不需要猜测 search 与详情时间字段的秒/毫秒关系。每次 RAG 最多增加 topK 个只读请求；Runtime 按 topK+1 预留调用额度。默认 SEARCH 清单仍保留原有严格核验行为。

当前离线检索与清单测试56项通过，配置确认与持久化启动另有集成验证。真实账户权限、响应兼容性、费用和内容质量尚未验证。
