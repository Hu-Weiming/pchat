...
更新日期：2026-09-24。当前阶段：Windows 0.1.1 最新访谈资料确认版已安装到D盘并通过安装探针；此前真实桌面问答、依据抽屉、多人停止与中断重开验收保留，针对一份失真的冻结萨特证据，新版生成规则重放通过。完整内容质量验收仍未通过。用户确认本机可用为交付目标，授权真实调用费用；新流程为一次选路、串行逐人物检索、等待全部返回后一次合并生成各人物回答/点评/总结。不能把链路成功视为内容质量通过。下方按日期记录，最新条目优先于早期接管快照。

### 2026-09-24 最新访谈确认版安装
...
- 安装前只读数据库确认没有RUNNING轮次；安装探针完成ListRoles、ListConversations、事件订阅和退出后Runtime清理，私有证据`D:/Dev/temp/pchat/installed-acceptance-5686632c-aa71-4f99-95b8-a2a05c4de80c/result.json`。未使用computer use或操作应用窗口；安装后真实应用保持关闭，用户可手动打开检查。没有修改环境或工具配置。

### 2026-09-24 直连知识库的访谈确认

- 资料确认页面增加“人物本人访谈”与片段使用的发言者署名；只有原典资料且署名非空时才可确认，确认值冻结在千帆文档manifest。直连检索在已验证文档和片段哈希后，仅接受所有发言标签均属于该人物的访谈文本，并标记INTERVIEW供回答与依据视图显示；采访者片段返回空结果而不冒充人物观点。
- 公开RAGPort测试与真实文件配置集成测试先失败再修复；缺少署名的确认失败且原配置不变。全量38文件376项测试、工作区/测试类型、依赖、portable及D盘暂存构建通过。尚未用用户实际上传的独立访谈文档进行在线验收；当前已安装版不含此最新资料确认入口。未修改环境或工具配置。
