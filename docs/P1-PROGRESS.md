...
更新日期：2026-09-12。当前阶段：队列、恢复、事件与安全边界通过定向验证，继续故障审计。
...
| FIFO、上下文冻结、幂等 | 进行中 | A/B/C、20 个并发重复命令、撤回、终态非法命令通过；补运行中设置变化的回归 |
| 停止、恢复、未知结果和明确重新生成 | 进行中 | 检索/生成迟到结果、重建旧运行者隔离、显式重做及保留旧草稿通过；提交/外发之间故障审计进行中 |
